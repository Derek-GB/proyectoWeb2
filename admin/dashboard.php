    ...
    <ul class="space-y-2">
      <?php foreach($list as $row): ?>
        <li class="p-3 bg-white rounded flex justify-between items-center">
          ...
          <div>
            <a href="propiedades.php?edit=<?= h($row['idPropiedad']) ?>" class="px-2 py-1 text-xs bg-yellow-100 rounded">Editar</a>
            <a href="propiedades.php?delete=<?= h($row['idPropiedad']) ?>" onclick="return confirm('Eliminar?')" class="px-2 py-1 text-xs bg-red-100 rounded">Eliminar</a>
          </div>
        </li>
      <?php endforeach; ?>
    </ul>
