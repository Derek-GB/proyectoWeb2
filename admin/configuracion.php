<?php
// Aquí puedes cambiar la configuración general de la página, como colores, imágenes y textos principales
...
<div class="p-6">
  ...
  <form method="post" enctype="multipart/form-data" class="bg-white p-4 rounded">
    ...
    <div class="grid grid-cols-2 gap-2">
      ...
      <label>Color Amarillo<input name="colorAmarillo" type="color" value="<?= h($config['colorAmarillo']) ?>"
          class="w-full p-1"></label>
    </div>
    <div class="grid grid-cols-2 gap-2 mt-2">
      <label>Color Gris<input name="colorGris" type="color" value="<?= h($config['colorGris']) ?>"
          class="w-full p-1"></label>
      <label>Color Blanco<input name="colorBlanco" type="color" value="<?= h($config['colorBlanco']) ?>"
          class="w-full p-1"></label>
    </div>
    <label class="block mt-2">Mensaje banner<textarea name="bannerMensaje"
        class="w-full p-2 rounded"><?= h($config['bannerMensaje']) ?></textarea></label>
    <label class="block mt-2">Texto Quiénes Somos<textarea name="quienesSomos"
        class="w-full p-2 rounded"><?= h($config['quienesSomos']) ?></textarea></label>
