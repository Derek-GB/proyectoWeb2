...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_property') {
    ...
    // Subidas: imagen destacada y mapa (ahora mapa es archivo)
    $img = uploadFile('imagen');           // imagen destacada
    $mapaImage = uploadFile('mapa_image'); // nueva imagen del mapa
    ...
    if ($id > 0) {
        ...
        if (!$mapaImage) $mapaImage = null; // puede ser null si no se sube mapa
        ...
        // tipos: s i s s d i s s s s => "sissdissss"
    ...
    <?php foreach($list as $r): ?>
      <li class="p-3 bg-white rounded flex justify-between items-center">
        ...
        <div>
          ...
          <a href="?delete=<?= h($r['idPropiedad']) ?>" onclick="return confirm('Eliminar?')" class="px-2 py-1 text-xs bg-red-100 rounded">Eliminar</a>
        </div>
      </li>
    <?php endforeach; ?>
  </ul>
